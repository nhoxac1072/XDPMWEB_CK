...
class productcontroller extends Controller
{
    ...
    function unactive_product($product_id) 
    {
        ...
        DB::table('product')->where('cateid',$product_id)->update(['status'=>1]);
        Session::put('message','thay đổi trạng thái mục sang hiện thành công');
        return \Redirect::to('allproduct');
    }
    function active_product($product_id)
    {
        DB::table('product')->where('cateid',$product_id)->update(['status'=>0]);
        Session::put('message','thay đổi trạng thái mục sang ẩn thành công');
        return \Redirect::to('allproduct');
    }
    function edit_product($product_id)
    {
        $editproduct= DB::table('product')->where('cateid',$product_id)->get(); //first laasy ra muc duy nhat or dau tien tim thay... get  thì mặc định có id r  nên lấy thôi
    ...
    public function update_product($product_id,Request $request)  
    {
        ...
        DB::table('product')->where('cateid',$product_id)->update($data); // where(id trong table, biến khởi tạo trên funcion)
    ...
    public function delete_product($product_id)
    {
        try
        {
            DB::table('product')->where('cateid',$product_id)->delete(); // where(id trong table, biến khởi tạo trên funcion)
            Session::put('message','xóa sản phẩm thành công');
            return \Redirect::to('allproduct');
        }
        catch(Exception $e )
        {
            Session::put('message','đã có loại sản phẩm tồn tại');
