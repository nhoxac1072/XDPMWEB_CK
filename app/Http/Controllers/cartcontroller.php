...
class cartcontroller extends Controller
{
    ...
    function storefromhome(Request $re)
    {
        ...
        {
            $showproduct=DB::table('showproduct')->where('id',$product_id)
            ...
            'weight' => 0,
            'options' => ['image' => $showproduct->image,'size' => $size,'thuonghieusp'=>$showproduct->name_dongSP]
             ));
    ...
    function incqtycart($product_id,$option)
    {
        $cart  = Cart::content();
        $rowId = $cart->where('id', $product_id)->where('options',$option)->first()->rowId;
        $row = Cart::get($rowId);
    ...
    function decqtycart($product_id,$option)
    {
        $cart = Cart::content();
        $rowId = $cart->where('id', $product_id)->where('options',$option)->first()->rowId;
        $row = Cart::get($rowId);
