...
class dongSPcontroller extends Controller
{
    ...
    function unactive_brand_product($brand_product_id) 
    {
        ...
        DB::table('dongsp')->where('brandid',$brand_product_id)->update(['status_dongSP'=>1]);
    ...
    {
        DB::table('dongsp')->where('brandid',$brand_product_id)->update(['status_dongSP'=>0]);
    ...
    {
        $edit_brandproduct= DB::table('dongsp')->where('brandid',$product_id)->get(); //first laasy ra muc duy nhat or dau tien tim thay... get  thì mặc định có id r  nên lấy thôi
    ...
    public function update_brand_product($product_id,Request $request)  
    {
        ...
        DB::table('dongsp')->where('brandid',$product_id)->update($data); // where(id trong table, biến khởi tạo trên funcion)
    ...
    public function delete_brand_product($product_id)
    {
        try
        {
            DB::table('dongsp')->where('brandid',$product_id)->delete(); // where(id trong table, biến khởi tạo trên funcion)
        ...
        catch(Exception $e )
        {
            // return Redirect::to('allbrand');
            // $e->getMessage();
            Session::put('message','dòng sản phẩm đang tôn tại');
            return \Redirect::to('allbrand');
        }
    }
}
